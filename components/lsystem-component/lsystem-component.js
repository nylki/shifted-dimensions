// This is a size optimized version of https://github.com/nylki/aframe-lsystem-component
...
import LSystem from './lindenmayer.es.js';
...
let lsystem = AFRAME.registerComponent('lsystem', {
  schema: {
  ...
  updateLSystem: function () {
    ...
    for (let p of this.data.productions) {
      this.LSystem.setProduction(p[0], p[1]);
    }
    this.LSystem.iterate(this.data.iterations);

    // in non-size-optimized official library we waited for async worker
    // to finish, here we resolve the promise immediately.
